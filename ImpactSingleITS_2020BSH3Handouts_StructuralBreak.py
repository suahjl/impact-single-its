...
# Event onsets
df.loc[df.index == date(2020, 7, 13), 'handout'] = 1  # 13 (OG), 1, 20, 24 (a few announcements)
for i in ['handout']:
    df.loc[df[i].isna(), i] = 0  # vectorised fillna
# Settings
event_choice = 'handout'
event_date = date(2020, 7, 13)  # 13 (OG), 1, 20, 24
...
bp = subprocess.call('\"C:\\Program Files\\R\\R-4.1.2\\bin\\Rscript.exe\" structuralbreak_2020BSH3Handouts.R',
                     shell=True)
telsendimg(conf=tel_config,
           path='Output/ImpactSingleITS_2020BSH3Handouts_StructuralBreak_Spending.png',
           cap='Bai-Perron Breakpoints: Spending')
telsendimg(conf=tel_config,
           path='Output/ImpactSingleITS_2020BSH3Handouts_StructuralBreak_MobRetl.png',
           cap='Bai-Perron Breakpoints: Mobility (Retail & Recreation)')
telsendimg(conf=tel_config,
           path='Output/ImpactSingleITS_2020BSH3Handouts_StructuralBreak_MobGroc.png',
